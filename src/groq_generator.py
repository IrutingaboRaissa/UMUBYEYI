"""Groq generation: general knowledge, not constrained to our own dataset.

Groq is the primary generator for wellbeing answers in both English and
...
Groq replaced Gemini as the external generator after Gemini's replies proved
low-quality in practice (short, tonally flat, occasionally inaccurate).
...
EPDS-10, progress dashboard), not the capstone's centerpiece. Groq answers
...
baby-care, off-topic routing in rag.py) regardless of what Groq says.
"""
...
class GroqGenerator:
    """Use Groq as the primary generator, never as the safety router."""
    ...
    def __init__(
        ...
        self.timeout = timeout if timeout is not None else float(
            os.environ.get("GROQ_TIMEOUT_SECONDS", "15")
        )
    ...
    def available(self) -> bool:
        return bool(self.api_key.strip()) and os.environ.get("UMU_DISABLE_GROQ") != "1"
    ...
    def _parse_json_field(raw: str, field: str) -> str:
        ...
        try:
            return str(json.loads(cleaned).get(field, "")).strip()
        except (json.JSONDecodeError, TypeError, AttributeError):
            return ""

    def _request(self, payload: dict) -> str:
        """Submit one chat-completion request and return candidate text.

        Sets an explicit User-Agent: Groq's API sits behind Cloudflare, which
        blocks urllib's default "Python-urllib/x.y" User-Agent outright
        (HTTP 403, Cloudflare error 1010) before the request ever reaches
        Groq -- confirmed by reproducing the block and clearing it with this
        header. Every call was silently failing closed on this until now.
        """
    ...
    def generate_social(self, query: str, lang: str) -> str:
        ...
        if not raw:
            return ""
        answer = self._parse_json_field(raw, "answer")
    ...
    def generate_title(self, user_message: str, bot_reply: str, lang: str) -> str:
        ...
        if not raw:
            return ""
        title = self._parse_json_field(raw, "title").strip("\"'")
    ...
    def generate(self, query: str, lang: str, history=None) -> str:
        ...
        Includes the last few real conversation turns so the model can hold
        an actual conversation (remember what was just said, not treat every
        message as the first one) -- a mother asking a wellbeing chatbot for
        help is already sharing sensitive content with this API on the
        current turn; withholding the last few turns of context bought
        little real privacy while making follow-ups read as disconnected."""
        self.last_error = ""
        if not self.available or lang not in {"en", "rw"}:
            self.last_error = "disabled, missing key, or unsupported language"
            return ""
