...
retrieval, deterministic safety rules, Umubyeyi's own fine-tuned generator (no third-party LLM produces
wellbeing-answer content), a real validated EPDS-10 wellness test, a client-side progress dashboard, a
guided check-in, mood history, and self-care support. It is a research prototype and does not diagnose
or replace a health professional.
...
- **wellbeing answers are generated by Umubyeyi's own fine-tuned model (BLOOMZ-560m,
  LoRA on real ESConv data) — no third-party LLM API produces that content**
...
- Gemini is used only for two low-stakes, non-clinical surfaces — greeting small talk
  and chat-list titles — never for wellbeing answer content
- strict generated-output validation with direct-passage fallback
...
Top-3 knowledge-base matches               non-diagnostic result + live SHAP explanation
       |
project-fine-tuned BLOOMZ generation (free-form, own weights, no evidence conditioning)
       |
local Ollama (dev-only, opt-in) -> reviewed retrieved passage (final fallback)
```
...
postpartum topics, which the deterministic safety/scope layer uses for routing and citation. Retrieval
finds the most relevant reviewed passages. The generator itself is Umubyeyi's own fine-tuned model
(BLOOMZ-560m, LoRA), which answers freely from its own weights the way a general-purpose fine-tuned
LLM would — it is not conditioned on or constrained to reproduce the retrieved evidence. Safety-critical
crisis and referral decisions remain fully deterministic and independent of any generator.

The fine-tuned generator was only trained and evaluated on English (see
`models/umubyeyi-bloomz-lora/training_manifest.json`'s `accepted_generation_languages`); no validated
Kinyarwanda conversational corpus exists yet, so Kinyarwanda replies come directly from the reviewed
retrieved passage — no generation, no third-party LLM. Gemini is used only for greeting small talk and
chat-list titles, both non-clinical, disclosed conveniences — never to produce wellbeing-answer content.
Conversation history (the last few turns) is now sent from the client and used both by the scope router
(so a bare follow-up like "okay how should I handle this" stays in context) and by the generator itself.
...
Current verified result: **74 tests passed**. Strategies include unit, parameterized, boundary-value,
...
follow-up stays in scope; an explicit off-topic pivot still redirects mid-conversation), an explicit
check that the wellbeing-answer cascade never calls Gemini, Gemini greeting/title failure fallback,
...
### Why the fine-tuned generator is not loaded by the current Vercel function

The LoRA adapter itself is compact (~3 MB), but it cannot run alone: inference must also load the
complete `bigscience/bloomz-560m` base checkpoint plus PyTorch, Transformers, PEFT and runtime memory.
The current Vercel Python function is deliberately lightweight, configured for a 45-second request and
excludes those local dependencies. The standard Python function bundle limit is 500 MB, so this model
stack does not fit that deployment path. See the
[Vercel Python runtime limits](https://vercel.com/docs/functions/runtimes/python).

Vercel introduced large Functions of up to 5 GB on Fluid Compute in June 2026, so this is no longer
categorically impossible. It would still require a separate deployment experiment covering large-function
...
5. Deploy using `vercel.json`; set a fresh `GEMINI_API_KEY` in project environment variables (used only
   for greeting small talk and chat titles — wellbeing answers do not depend on it). The hosted build
...
- No human-authored Kinyarwanda response corpus is used for generator fine-tuning. Kinyarwanda wellbeing
  answers therefore come directly from the reviewed retrieved passage, not generation.
- At 560M parameters and ~7,800 fine-tuning examples, the generator's own English output quality is real
  but modest (ROUGE-L 0.1285) — occasionally topically present but tonally imperfect, especially on
  later conversation turns. This is an honest, measured limitation of a small fine-tuned model, not a
  routing bug.
- Gemini is an externally hosted model, used only for greeting small talk and chat titles — never for
  wellbeing-answer content. Sending a message to it requires clear user disclosure, data minimization,
...
human-authored Kinyarwanda conversational corpus for generator fine-tuning, a validated Kinyarwanda
EPDS-10 translation, and hosting the fine-tuned generator (for example via Hugging Face Spaces) so it
also runs in the deployed build.
