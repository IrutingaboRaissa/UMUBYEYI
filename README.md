...
Bilingual English/Kinyarwanda postpartum emotional-well-being assistant for first-time Rwandan mothers,
combining trained screening-risk classifiers, live SHAP explainability, source-grounded retrieval, and
general-knowledge AI chat generation. It is a research prototype and does not diagnose or replace a
health professional.

Live app: https://firstmumassist-six.vercel.app/
...
recording a demo — it redeploys automatically on every push to `main`.

## Demo Video

https://www.loom.com/share/957c6c864cd940009977ff87f513b637
...
- [Testing Strategies & Results](#testing-strategies--results)
- [Analysis of Results](#analysis-of-results)
- [Deployment Plan & Execution](#deployment-plan--execution)
- [Recommendations & Future Work](#recommendations--future-work)

## About the Project

Umubyeyi is a final-year BSc Software Engineering capstone project (African Leadership University,
Kigali). It addresses a real gap: first-time mothers in Rwanda need accessible, bilingual emotional
support in the first six months after childbirth, but that support is not always available on demand,
and existing digital tools rarely operate in Kinyarwanda or ground their answers in reviewed evidence.

Umubyeyi provides a free-text chat that routes deterministically for safety before any generation
happens, a validated EPDS-10 postpartum wellness self-assessment, a guided screening-risk check-in with
live SHAP explainability, a client-side progress dashboard, and self-care support — all bilingual.

**Author:** Raissa Irutingabo — BSc Software Engineering, African Leadership University, Kigali
...
### Guided check-in & explainability
...
### EPDS-10 wellness test & progress dashboard
...
deterministic and are not presented as learned output.

### Step 6 — Start the complete local application
...
- ethical Rwanda-specific data collection to replace/augment the Bangladesh-sourced screening dataset
- professional and native-speaker review of Kinyarwanda passages, and a formal Rwandan user study
- usability testing, model calibration, and fairness analysis
- multilingual semantic retrieval (beyond character n-gram TF-IDF)
- a human-authored Kinyarwanda conversational corpus so the project's own fine-tuned generator can
  cover Kinyarwanda too, not just the external LLM providers
- a validated Kinyarwanda EPDS-10 translation
- hosting the fine-tuned generator in the deployed build (for example via a paid always-on host, since
  free-tier Hugging Face Spaces storage limits ruled that out during this project) so its fallback also
  runs on Vercel, not just locally
