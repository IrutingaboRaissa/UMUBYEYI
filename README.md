...
- wellbeing answers are generated by grounded Gemini first (reliable in both English and
  Kinyarwanda), with Umubyeyi's own fine-tuned model (BLOOMZ-560m, LoRA on real ESConv
  data) as a real, documented fallback layer, then optional local Ollama, then the
...
- strict generated-output validation (coverage + evidence-grounding, one corrective
  retry) with direct-passage fallback
...
Top-3 knowledge-base matches               non-diagnostic result + live SHAP explanation
       |
grounded Gemini generation (evidence-constrained, strict local validation, one retry)
       |
...
finds the most relevant reviewed passages.

The generation cascade tries Gemini first: it receives only the current message and the
classifier-selected reviewed evidence (never raw conversation history, which can contain sensitive
details), and its structured JSON response must pass a strict local check -- every concern it claims to
address must be covered, every factual claim must be traceable to the supplied evidence by wording
overlap, and any new number must already appear in the evidence -- before its answer is used, with one
corrective retry on failure. Gemini was restored to the primary role after the project's own fine-tuned
model proved unreliable mid-conversation in practice; the fine-tuned model (BLOOMZ-560m, LoRA on real
ESConv data, answering freely from its own weights) remains a real, working fallback layer, not a
decorative one, exercised whenever Gemini's answer fails validation or the API is unavailable.
...
- Gemini is an externally hosted model, used as the primary generator for wellbeing answers in both
  languages, plus greeting small talk and chat titles. Its wellbeing answers are constrained to the
  classifier-selected reviewed evidence and must pass a strict local coverage/grounding check (with one
  corrective retry) before being used — it cannot introduce facts, phone numbers, or doses the evidence
  doesn't contain. Sending a message to it requires clear user disclosure, data minimization, secure key
